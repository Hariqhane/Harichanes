...
class MetropolisHastings(object):
    ...
    def __iter__(self) -> Iterator[Tensor]:
        ...
        while True:
            # y ~ q(y | x)
            y = self.q(x).sample().cuda()

            # log f(y)
            log_f_y = self.log_f(y).cuda()

            #     f(y)   q(x | y)
            # a = ---- * --------
            #     f(x)   q(y | x)
            log_a = log_f_y.cuda() - log_f_x.cuda()

            if not self.symmetric:
                log_a = log_a.cuda() + self.q(y).log_prob(x).cuda() - self.q(x).log_prob(y).cuda()

            a = log_a.exp().cuda()

            # u in [0; 1]
            u = torch.rand(a.shape).to(a).cuda()

            # if u < a, x <- y
            # else x <- x
            mask = u < a

            x = torch.where(mask.unsqueeze(-1), y, x).cuda()
            log_f_x = torch.where(mask, log_f_y, log_f_x).cuda()
